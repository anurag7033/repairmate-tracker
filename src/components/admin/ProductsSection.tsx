...
const ProductsSection = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      let payload = form;
      // If admin never wrote/generated a description, create one automatically.
      if (!payload.description.trim()) {
        let auto = "";
        try {
          auto = await generateProductDescription({
            name: payload.name,
            brand: payload.brand,
            category: payload.category,
            sellingPrice: payload.sellingPrice,
            finalPrice: finalPreview,
          });
        } catch {
          auto = defaultProductDescription({
            name: payload.name,
            brand: payload.brand,
            category: payload.category,
            finalPrice: finalPreview,
        ...
        payload = { ...payload, description: auto };
      }
      if (editingId) {
    ...
    try {
      ...
      setForm((f) => ({
        ...
        description: defaultProductDescription({
          name: f.name, brand: f.brand, category: f.category, finalPrice: finalPreview,
        }),
      }));
      toast({ title: "Used default description", description: err.message, variant: "destructive" });
    } finally {
      setAiBusy(false);
    }
  };
